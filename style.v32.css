/*
  ERP Escolar — Aplicação principal
  Arquivo extraído da base v32 sem alterar lógica funcional.
  Contém o objeto global window.app e todos os fluxos existentes.
*/
...
const app = {
  ...
  buildParentChildDocument(kind = 'ficha') {
    ...
    else body = infoBlocks.aluno + infoBlocks.escola + infoBlocks.responsaveis + infoBlocks.autorizacoes + infoBlocks.presenca;
    ...
    modal.innerHTML = `
      <div class="parent-doc-topbar">
        ...
        <div class="parent-doc-actions">
          <button type="button" onclick="window.app.shareParentChildDocument()" aria-label="Compartilhar ficha">
            <i data-lucide="share-2" class="w-4 h-4"></i>
            <span>Compartilhar</span>
          </button>
          <button type="button" onclick="window.app.printParentChildDocument()" aria-label="Baixar ou imprimir ficha em PDF">
  ...
  renderParentView() {
    ...
    const ultimaConquista = conquistas[0] || null;
    const ultimaAtividade = atividades[0] || null;
    const atividadesFeitas = atividades.filter(a => a.feita !== false).length;
    const atividadesPendentes = Math.max(atividades.length - atividadesFeitas, 0);
    const aniversarioResumo = aniversarioInfo ? (aniversarioInfo.hoje ? 'Aniversaria hoje 🎂' : aniversarioInfo.label) : 'Nascimento não informado';
    const sinaisEscolares = conquistas.length + atividades.length + metricas.totalChamadas;
    const sinaisTexto = sinaisEscolares ? `${sinaisEscolares} registro(s) acompanhados` : 'Primeiros registros em construção';
    const nomeCurtoAluno = (() => {
      const partes = String(s.name || 'Aluno').trim().split(/\s+/).filter(Boolean);
      if (partes.length <= 1) return partes[0] || 'Aluno';
      return `${partes[0]} ${partes[partes.length - 1]}`;
    })();
    const presencaResumoCurto = metricas.totalChamadas
      ? `${metricas.presencaPct}%`
      : '—';
    const presencaSubtextoCurto = metricas.totalChamadas
      ? `${metricas.presencas} pres. · ${metricas.faltas} falt.`
      : 'Sem chamada';
    const conquistaResumoCurto = conquistas.length ? String(conquistas.length) : '—';
    const conquistaSubtextoCurto = ultimaConquista
      ? (getAchievementConfig(ultimaConquista.tipo).shortLabel || getAchievementConfig(ultimaConquista.tipo).label || 'Recente')
      : 'Sem registros';
    const atividadeResumoCurto = atividades.length ? String(atividades.length) : '—';
    const atividadeSubtextoCurto = ultimaAtividade
      ? `${ultimaAtividade.feita === false ? 'Pendente' : 'Feita'} · ${formatarDataBR(ultimaAtividade.date)}`
      : 'Sem registros';
    ...
    const filhoSection = `
      <section class="parent-v36-child-page">
        <header class="parent-v36-hero">
          <div class="parent-v36-avatar">
            <img src="${escapeAttr(safeUrl(s.photo) || criarAvatarPadrao(s.name))}" alt="${escapeAttr(s.name)}">
            <span><i data-lucide="star" class="w-4 h-4"></i></span>
          </div>

          <div class="parent-v36-identity">
            <small>Perfil escolar</small>
            <h2>${escapeHTML(nomeCurtoAluno)}</h2>
            <p>
              <span>${escapeHTML(idadeTexto)}</span>
              <span>${escapeHTML(s.turma_nome || 'Turma não informada')}</span>
              <span>${escapeHTML(professoraTexto)}</span>
            </p>
            <div class="parent-v36-badges">
              <span><i data-lucide="cake" class="w-4 h-4"></i>${escapeHTML(aniversarioResumo)}</span>
              <span><i data-lucide="calendar-days" class="w-4 h-4"></i>${escapeHTML(nascimentoTexto)}</span>
            </div>
          </div>

          <div class="parent-v36-actions">
            ${this.data.students.length > 1 ? `
              <details class="parent-v36-switcher">
                <summary>
                ...
                <div>
                  ${this.data.students.map(child => {
                    const childNameParts = String(child.name || 'Aluno').trim().split(/\s+/).filter(Boolean);
                    const childShortName = childNameParts.length > 1 ? `${childNameParts[0]} ${childNameParts[childNameParts.length - 1]}` : (childNameParts[0] || 'Aluno');
                    return `
                      <button type="button" onclick="window.app.setActiveParentChild('${escapeAttr(child.id)}', 'filho')" class="${String(child.id) === String(s.id) ? 'is-active' : ''}">
                        <img src="${escapeAttr(safeUrl(child.photo) || criarAvatarPadrao(child.name))}" alt="${escapeAttr(child.name)}">
                        <span>
                          <strong>${escapeHTML(childShortName)}</strong>
            ...
            <button type="button" onclick="window.app.contactTeacherFromParent()" class="parent-v36-action-btn" aria-label="Falar com a professora">
        ...
        <section class="parent-v36-metrics" aria-label="Resumo principal">
          <article class="${presencaClass}">
            <span><i data-lucide="calendar-check-2" class="w-5 h-5"></i></span>
            <div>
              <small>Presença</small>
              <strong>${escapeHTML(presencaResumoCurto)}</strong>
              <em>${escapeHTML(presencaSubtextoCurto)}</em>
            </div>
          </article>
          <article class="is-award">
            <span><i data-lucide="trophy" class="w-5 h-5"></i></span>
            <div>
              <small>Conquistas</small>
              <strong>${escapeHTML(conquistaResumoCurto)}</strong>
              <em>${escapeHTML(conquistaSubtextoCurto)}</em>
            </div>
          </article>
          <article class="is-blue">
            ...
            <div>
              <small>Atividades</small>
              <strong>${escapeHTML(atividadeResumoCurto)}</strong>
              <em>${escapeHTML(atividadeSubtextoCurto)}</em>
            </div>
          </article>
        </section>

        <main class="parent-v36-layout">
          <section class="parent-v36-feed">
            <article class="parent-v36-card">
              <div class="parent-v36-card-head">
                <span><i data-lucide="sparkles" class="w-5 h-5"></i></span>
                <div>
                  <small>Reconhecimentos</small>
                  <h3>Conquistas recentes</h3>
                </div>
              </div>

              <div class="parent-v36-list">
                ${conquistas.length ? conquistas.slice(0, 3).map((a, index) => {
                  const config = getAchievementConfig(a.tipo);
                  const colorClass = index % 3 === 0 ? 'is-sky' : index % 3 === 1 ? 'is-emerald' : 'is-amber';
                  return `
                    <div class="parent-v36-list-item ${colorClass}">
                  ...
                  <div class="parent-v36-empty">
                    <i data-lucide="star" class="w-5 h-5"></i>
                    <span>Nenhuma conquista registrada ainda.</span>
                  </div>
                `}
              </div>
            </article>
            ...
            <article class="parent-v36-card">
              <div class="parent-v36-card-head">
                <span><i data-lucide="notebook-tabs" class="w-5 h-5"></i></span>
                <div>
              ...
              <div class="parent-v36-list">
                ${atividades.length ? atividades.slice(0, 3).map(a => `
                  <div class="parent-v36-list-item ${a.feita === false ? 'is-warn' : 'is-emerald'}">
                    <span><i data-lucide="${a.feita === false ? 'circle-alert' : 'check-circle-2'}" class="w-5 h-5"></i></span>
                    <div>
          ...
          <aside class="parent-v36-side">
            <article class="parent-v36-card parent-v36-doc-card">
              <div class="parent-v36-card-head">
              ...
              <div class="parent-v36-doc-list">
                <button type="button" onclick="window.app.openParentChildDocument('ficha')">
                  <i data-lucide="id-card" class="w-5 h-5"></i>
                  <span>
                    <strong>Ficha completa</strong>
                    <small>Dados escolares e familiares</small>
                  </span>
                  <i data-lucide="chevron-right" class="w-4 h-4"></i>
                </button>
                <button type="button" onclick="window.app.openParentChildDocument('responsaveis')">
                  <i data-lucide="heart-handshake" class="w-5 h-5"></i>
                  <span>
                    <strong>Responsáveis</strong>
                    <small>Contatos e autorizações</small>
                  </span>
                  <i data-lucide="chevron-right" class="w-4 h-4"></i>
                </button>
              </div>
