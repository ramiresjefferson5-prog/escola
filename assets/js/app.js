...
const app = {
  ...
  renderParentView() {
    ...
    const filhoSection = `
      <section class="parent-v35-child-page">
        <article class="parent-v35-child-hero">
          <div class="parent-v35-child-photo">
            <img src="${escapeAttr(safeUrl(s.photo) || criarAvatarPadrao(s.name))}" alt="${escapeAttr(s.name)}">
            <span><i data-lucide="graduation-cap" class="w-5 h-5"></i></span>
          </div>

          <div class="parent-v35-child-copy">
            <small>Perfil do filho</small>
            <h2>${escapeHTML(s.name)}</h2>
            <p>${escapeHTML(idadeTexto)} · ${escapeHTML(s.turma_nome || 'Turma não informada')}</p>

            <div class="parent-v35-meta-line">
              <span><i data-lucide="cake" class="w-4 h-4"></i>${escapeHTML(nascimentoTexto)}</span>
              <span><i data-lucide="school" class="w-4 h-4"></i>${escapeHTML(professoraTexto)}</span>
              <span><i data-lucide="sparkles" class="w-4 h-4"></i>${escapeHTML(aniversarioResumo)}</span>
            </div>
          </div>

          <div class="parent-v35-child-actions">
            ${this.data.students.length > 1 ? `
              <details class="parent-v35-switcher">
                <summary>
            ...
            <button type="button" onclick="window.app.contactTeacherFromParent()" class="parent-v35-soft-btn">
        ...
        <section class="parent-v35-kpis" aria-label="Resumo escolar principal">
          <article class="parent-v35-kpi ${presencaClass}">
          ...
          <article class="parent-v35-kpi is-award">
            <span><i data-lucide="trophy" class="w-5 h-5"></i></span>
            <div>
              <small>Conquistas</small>
              <strong>${conquistas.length}</strong>
              <em>${ultimaConquista ? escapeHTML(getAchievementConfig(ultimaConquista.tipo).shortLabel || getAchievementConfig(ultimaConquista.tipo).label) : 'Sem registros'}</em>
            </div>
          </article>

          <article class="parent-v35-kpi is-blue">
            <span><i data-lucide="book-open-check" class="w-5 h-5"></i></span>
            <div>
              <small>Atividades</small>
              <strong>${atividades.length}</strong>
        ...
        <section class="parent-v35-content">
          <article class="parent-v35-panel parent-v35-profile-panel">
            <div class="parent-v35-section-title">
            ...
            <div class="parent-v35-info-list">
              <div><span>Turma</span><strong>${escapeHTML(s.turma_nome || 'Não informada')}</strong></div>
              <div><span>Professora</span><strong>${escapeHTML(professoraTexto)}</strong></div>
              <div><span>Nascimento</span><strong>${escapeHTML(nascimentoTexto)}</strong></div>
              <div><span>Aniversário</span><strong>${escapeHTML(aniversarioResumo)}</strong></div>
            </div>

            <div class="parent-v35-doc-actions">
              <button type="button" onclick="window.app.openParentChildDocument('ficha')" class="is-primary">
                <i data-lucide="file-text" class="w-5 h-5"></i>
                <span>
                  <strong>Ver ficha completa</strong>
                  <small>Documento escolar em página completa</small>
                </span>
              </button>
              <button type="button" onclick="window.app.openParentChildDocument('responsaveis')">
                ...
                <span>
                  ...
                  <small>Contatos, autorizações e saúde</small>
                </span>
              </button>
            </div>
          </article>

          <article class="parent-v35-panel parent-v35-achievements">
            <div class="parent-v35-section-title">
            ...
            <div class="parent-v35-feed-list">
              ${conquistas.length ? conquistas.slice(0, 4).map((a, index) => {
                const config = getAchievementConfig(a.tipo);
                const colorClass = index % 4 === 0 ? 'is-sky' : index % 4 === 1 ? 'is-emerald' : index % 4 === 2 ? 'is-violet' : 'is-amber';
                return `
                  <div class="parent-v35-feed-item ${colorClass}">
          ...
          <article class="parent-v35-panel parent-v35-activities">
            <div class="parent-v35-section-title">
            ...
            <div class="parent-v35-activity-list">
