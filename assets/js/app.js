...
const app = {
  ...
  renderParentView() {
    ...
    const filhoSection = `
      <section class="parent-v34-child-page">
        <article class="parent-v34-child-hero">
          <div class="parent-v34-hero-bg" aria-hidden="true"></div>

          <div class="parent-v34-hero-main">
            <div class="parent-v34-photo-stack">
              <img src="${escapeAttr(safeUrl(s.photo) || criarAvatarPadrao(s.name))}" alt="${escapeAttr(s.name)}">
              <span class="parent-v34-photo-badge"><i data-lucide="school" class="w-4 h-4"></i></span>
            </div>

            <div class="parent-v34-hero-copy">
              <small>Perfil escolar</small>
              <h2>${escapeHTML(s.name)}</h2>
              <p>${escapeHTML(idadeTexto)} · ${escapeHTML(s.turma_nome || 'Turma não informada')} · ${escapeHTML(professoraTexto)}</p>

              <div class="parent-v34-hero-actions">
                ${this.data.students.length > 1 ? `
                  <details class="parent-v34-switcher">
                    <summary>
                      <i data-lucide="repeat-2" class="w-4 h-4"></i>
                      <span>Trocar filho</span>
                    </summary>
                    <div>
                      ${this.data.students.map(child => `
                        <button type="button" onclick="window.app.setActiveParentChild('${escapeAttr(child.id)}', 'filho')" class="${String(child.id) === String(s.id) ? 'is-active' : ''}">
                ...
                <button type="button" onclick="window.app.contactTeacherFromParent()" class="parent-v34-action-btn">
                  <i data-lucide="message-circle-heart" class="w-4 h-4"></i>
                  <span>Professora</span>
                </button>

                <button type="button" onclick="window.app.setParentPanelTab('fotos')" class="parent-v34-action-btn">
        ...
        <section class="parent-v34-insight-row" aria-label="Indicadores principais">
          <article class="parent-v34-kpi ${presencaClass}">
          ...
          <article class="parent-v34-kpi is-award">
            <span><i data-lucide="trophy" class="w-5 h-5"></i></span>
            <div>
              <small>Conquistas</small>
              <strong>${conquistas.length}</strong>
              <em>${ultimaConquista ? escapeHTML(getAchievementConfig(ultimaConquista.tipo).shortLabel || getAchievementConfig(ultimaConquista.tipo).label) : 'Sem registros ainda'}</em>
            </div>
          </article>

          <article class="parent-v34-kpi is-blue">
            <span><i data-lucide="book-open-check" class="w-5 h-5"></i></span>
            <div>
              <small>Atividades</small>
              <strong>${atividades.length}</strong>
              <em>${atividadesPendentes ? `${atividadesPendentes} pendente(s)` : (atividades.length ? 'Em dia' : 'Sem registros')}</em>
            </div>
          </article>
        </section>

        <section class="parent-v34-flow">
          <article class="parent-v34-main-panel">
            <div class="parent-v34-section-head">
              <span><i data-lucide="sparkles" class="w-5 h-5"></i></span>
              <div>
                <small>Leitura rápida</small>
                <h3>Acompanhamento de ${escapeHTML(primeiroNomeAluno)}</h3>
              </div>
            </div>

            <div class="parent-v34-reading-card">
              <p>
                ${metricas.totalChamadas
                  ? `Presença geral em <strong>${metricas.presencaPct}%</strong>, com ${metricas.presencas} presença(s) e ${metricas.faltas} falta(s).`
                  : 'A chamada ainda não possui registros suficientes para análise.'}
                ${conquistas.length
                  ? ` Há <strong>${conquistas.length}</strong> conquista(s) registrada(s) no percurso escolar.`
                  : ' As conquistas aparecerão aqui quando forem publicadas pela professora.'}
              </p>

              <div class="parent-v34-bars">
                <div>
                  <span><b>Presença</b><em>${metricas.presencaPct}%</em></span>
                  <div><i style="width:${Math.max(0, Math.min(100, metricas.presencaPct))}%"></i></div>
                </div>
                <div>
                  <span><b>Comportamento positivo</b><em>${metricas.comportamentoPct}%</em></span>
                  <div><i style="width:${Math.max(0, Math.min(100, metricas.comportamentoPct))}%"></i></div>
                </div>
              </div>
            </div>

            <div class="parent-v34-mini-facts">
              <div>
                <small>Professor(a)</small>
                <strong>${escapeHTML(professoraTexto)}</strong>
              </div>
              <div>
                <small>Aniversário</small>
                <strong>${escapeHTML(aniversarioResumo)}</strong>
              </div>
              <div>
                <small>Registros</small>
                <strong>${escapeHTML(sinaisTexto)}</strong>
              </div>
            </div>
          </article>

          <aside class="parent-v34-side-panel">
            <button type="button" onclick="window.app.openParentChildDocument('ficha')" class="parent-v34-doc-button is-primary">
              <i data-lucide="id-card" class="w-5 h-5"></i>
              <span>
                <strong>Ficha completa</strong>
                <small>Abrir página moderna para visualizar ou compartilhar</small>
              </span>
              <em><i data-lucide="arrow-up-right" class="w-4 h-4"></i></em>
            </button>

            <button type="button" onclick="window.app.openParentChildDocument('responsaveis')" class="parent-v34-doc-button">
              <i data-lucide="heart-handshake" class="w-5 h-5"></i>
              <span>
                <strong>Responsáveis e contatos</strong>
                <small>Dados autorizados e informações importantes</small>
              </span>
              <em><i data-lucide="chevron-right" class="w-4 h-4"></i></em>
            </button>
          </aside>
        </section>

        <section class="parent-v34-social-grid">
          <article class="parent-v34-timeline-panel">
            <div class="parent-v34-section-head">
              <span><i data-lucide="badge-check" class="w-5 h-5"></i></span>
              <div>
            ...
            <div class="parent-v34-achievement-feed">
              ${conquistas.length ? conquistas.slice(0, 5).map((a, index) => {
                const config = getAchievementConfig(a.tipo);
                const colorClass = index % 4 === 0 ? 'is-sky' : index % 4 === 1 ? 'is-emerald' : index % 4 === 2 ? 'is-violet' : 'is-amber';
                return `
                  <div class="parent-v34-achievement ${colorClass}">
                ...
                <div class="parent-v34-empty-line">
                  <i data-lucide="star" class="w-5 h-5"></i>
                  <span>Sem conquistas registradas ainda.</span>
                </div>
              `}
            </div>
          </article>
          ...
          <article class="parent-v34-timeline-panel">
            <div class="parent-v34-section-head">
              ...
              <div>
                <small>Registros pedagógicos</small>
                <h3>Atividades recentes</h3>
              </div>
            </div>

            <div class="parent-v34-activity-feed">
              ${atividades.length ? atividades.slice(0, 4).map(a => `
                <div>
